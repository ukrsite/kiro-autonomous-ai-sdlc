...
- **If approved**: Proceed to Step 11 (WF Classification)
...
## Step 11: Classify Request Type and Select WF

**MANDATORY**: After user approves the execution plan, classify the request and select the WF for CONSTRUCTION.

**Classification rules** (based on requirements + intent analysis):
| Request Type | Selected WF |
|---|---|
| New Feature, Enhancement | wf1-requirement-to-software |
| Refactoring | wf2-autonomous-refactoring |
| Upgrade, Migration | wf3-dependency-upgrades |
| Bug Fix | wf4-bug-fix |
| Documentation | wf5-documentation |

Record:
- `selected_wf`: the WF identifier
- `request_type`: the classified type
- `confidence`: high / medium / low
- `rationale`: brief explanation of why this WF was selected

Present the selection to the user for confirmation. Allow override. Record any override in the audit log.

## Step 12: Generate Handoff Artifact

**MANDATORY**: Create `aidlc-docs/inception/plans/workflow-handoff.md` using this exact structure:

```markdown
# Workflow Handoff Artifact

## Selected Workflow
- **Workflow**: {wf_identifier}
...
|---|---|
| Functional Design | {delegated to {wf_identifier}|not delegated} |
| NFR Requirements | {delegated to {wf_identifier}|not delegated} |
| NFR Design | {delegated to {wf_identifier}|not delegated} |
| Infrastructure Design | {delegated to {wf_identifier}|not delegated} |
...
- **Original Selection**: {original_wf or N/A}
- **Override To**: {overridden_wf or N/A}
- **Override Rationale**: {rationale or N/A}
```

**Delegation mapping** — apply `[D]` markers in `aidlc-state.md` for these stages per selected WF:
...
Update `aidlc-state.md` with a `## Workflow Integration` section:
```markdown
## Workflow Integration
- **Selected WF**: {wf_identifier}
- **Request Type**: {classification}
- **Handoff Timestamp**: {ISO 8601}
- **Integration Status**: Handoff pending
```

## Step 13: Delegate to Selected WF

**MANDATORY**: After creating the Handoff Artifact, STOP all AI-DLC CONSTRUCTION stage execution and delegate to the selected WF.

**CRITICAL RULE**: AI-DLC must NOT execute any CONSTRUCTION stage (NFR Requirements, NFR Design, Functional Design, Infrastructure Design, Code Generation, Build and Test) directly. These are delegated to the WF.

**Delegation message to present to user**:

```markdown
# 📦 INCEPTION Complete — Handing off to {WF_NAME}

INCEPTION phase is complete. The Handoff Artifact has been created at:
`aidlc-docs/inception/plans/workflow-handoff.md`

**Selected Workflow**: {wf_identifier}
**Reason**: {classification_rationale}

**CONSTRUCTION is now delegated to {WF_NAME}**, which will:
{brief description of what the selected WF does, e.g. for WF1: "generate design.md + tasks.md from the INCEPTION requirements, implement code, run code review / test coverage / security scan checkpoints, then merge"}

The selected WF will load the Handoff Artifact and use the INCEPTION requirements as its authoritative input — no repeated planning.

> ✅ **Ready to start {WF_NAME}?** Reply "yes" to begin CONSTRUCTION.
```

**On user confirmation**: Invoke the selected WF skill by loading and following its SKILL.md from `.kiro/skills/developer-skills/{wf_identifier}/SKILL.md`. The WF will detect the Handoff Artifact and skip its own requirements gathering step.
...
```markdown
## Workflow Planning - Approval and Handoff
**Timestamp**: [ISO timestamp]
**User Input**: "[User's COMPLETE RAW response]"
**AI Response**: "Execution plan approved. Classified as {request_type} → selected {wf_identifier}. Handoff Artifact created. Delegating CONSTRUCTION to {wf_identifier}."
**Context**: INCEPTION complete. Handoff to {wf_identifier}.
